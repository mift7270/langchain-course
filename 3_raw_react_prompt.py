...
def run_agent(question: str):
    ...
    for iteration in range(1, MAX_ITERATONS + 1):
        ...
        print(f" [Parsing] looking for final Answer in LLM ouput....")
        ...
        print(f" [Parsing] looking for Action and Action input in LLM Ouput...")

        action_match = re.search(r"Action:\s*(.+)", output)
        action_input_match = re.search(r"Action Input:\s*(.+)", output)

        if not action_match or not action_input_match:
            print("[Parsing] ERROR: Could not parse Action/Action Input from LLM Ouput")
            break
